...
class RuleConfig 
{
    ...
    public function route(string $name, array $value = []): \Symfony\Config\FosHttpCache\Invalidation\RuleConfig\RouteConfig
    {
        if (!isset($this->routes[$name])) {
            ...
            return $this->routes[$name] = new \Symfony\Config\FosHttpCache\Invalidation\RuleConfig\RouteConfig($value);
        }
        if ([] === $value) {
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('routes', $value)) {
            $this->_usedProperties['routes'] = true;
            $this->routes = array_map(function ($v) { return new \Symfony\Config\FosHttpCache\Invalidation\RuleConfig\RouteConfig($v); }, $value['routes']);
            unset($value['routes']);
        }
