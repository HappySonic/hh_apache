...
class MassiveArtBuildConfig implements \Symfony\Component\Config\Builder\ConfigBuilderInterface
{
     ...
     * @return $this
     */
    public function commandClass($value): self
    {
    ...
    public function targets(string $name, array $value = []): \Symfony\Config\MassiveArtBuild\TargetsConfig
    {
        if (!isset($this->targets[$name])) {
            ...
            return $this->targets[$name] = new \Symfony\Config\MassiveArtBuild\TargetsConfig($value);
        }
        if ([] === $value) {
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('targets', $value)) {
            $this->_usedProperties['targets'] = true;
            $this->targets = array_map(function ($v) { return new \Symfony\Config\MassiveArtBuild\TargetsConfig($v); }, $value['targets']);
            unset($value['targets']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['targets'])) {
            $output['targets'] = array_map(function ($v) { return $v->toArray(); }, $this->targets);
        }
