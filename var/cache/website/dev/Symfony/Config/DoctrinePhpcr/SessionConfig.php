...
class SessionConfig 
{
    ...
    public function session(string $name, array $value = []): \Symfony\Config\DoctrinePhpcr\Session\SessionConfig
    {
        if (!isset($this->sessions[$name])) {
            ...
            return $this->sessions[$name] = new \Symfony\Config\DoctrinePhpcr\Session\SessionConfig($value);
        }
        if ([] === $value) {
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('sessions', $value)) {
            $this->_usedProperties['sessions'] = true;
            $this->sessions = array_map(function ($v) { return new \Symfony\Config\DoctrinePhpcr\Session\SessionConfig($v); }, $value['sessions']);
