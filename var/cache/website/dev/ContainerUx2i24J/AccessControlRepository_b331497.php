...
class AccessControlRepository_b331497 extends \Sulu\Bundle\SecurityBundle\Entity\AccessControlRepository implements \ProxyManager\Proxy\VirtualProxyInterface
{
    ...
    {
        $this->initializer853e4 && ($this->initializer853e4->__invoke($valueHolder4a50a, $this, 'findByTypeAndIdAndRole', array('type' => $type, 'id' => $id, 'roleId' => $roleId), $this->initializer853e4) || 1) && $this->valueHolder4a50a = $valueHolder4a50a;

        return $this->valueHolder4a50a->findByTypeAndIdAndRole($type, $id, $roleId);
    }

    public function findByTypeAndId($type, $id, $system = null)
    {
        $this->initializer853e4 && ($this->initializer853e4->__invoke($valueHolder4a50a, $this, 'findByTypeAndId', array('type' => $type, 'id' => $id, 'system' => $system), $this->initializer853e4) || 1) && $this->valueHolder4a50a = $valueHolder4a50a;

        return $this->valueHolder4a50a->findByTypeAndId($type, $id, $system);
    }
    ...
    {
        $this->initializer853e4 && ($this->initializer853e4->__invoke($valueHolder4a50a, $this, 'findIdsWithGrantedPermissions', array('user' => $user, 'permission' => $permission, 'entityClass' => $entityClass, 'entityIds' => $entityIds, 'system' => $system, 'anonymousRoleId' => $anonymousRoleId), $this->initializer853e4) || 1) && $this->valueHolder4a50a = $valueHolder4a50a;

        return $this->valueHolder4a50a->findIdsWithGrantedPermissions($user, $permission, $entityClass, $entityIds, $system, $anonymousRoleId);
    }
