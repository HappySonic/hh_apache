...
class AccessControlRepository_b331497 extends \Sulu\Bundle\SecurityBundle\Entity\AccessControlRepository implements \ProxyManager\Proxy\VirtualProxyInterface
{
    /**
     * @var \Sulu\Bundle\SecurityBundle\Entity\AccessControlRepository|null wrapped object, if the proxy is initialized
     */
    ...
    {
        $this->initializerb5509 && ($this->initializerb5509->__invoke($valueHolderedeb5, $this, 'findByTypeAndIdAndRole', array('type' => $type, 'id' => $id, 'roleId' => $roleId), $this->initializerb5509) || 1) && $this->valueHolderedeb5 = $valueHolderedeb5;

        return $this->valueHolderedeb5->findByTypeAndIdAndRole($type, $id, $roleId);
    }

    public function findByTypeAndId($type, $id, $system = null)
    {
        $this->initializerb5509 && ($this->initializerb5509->__invoke($valueHolderedeb5, $this, 'findByTypeAndId', array('type' => $type, 'id' => $id, 'system' => $system), $this->initializerb5509) || 1) && $this->valueHolderedeb5 = $valueHolderedeb5;

        return $this->valueHolderedeb5->findByTypeAndId($type, $id, $system);
    }

    public function findIdsWithGrantedPermissions(?\Sulu\Component\Security\Authentication\UserInterface $user, int $permission, string $entityClass, array $entityIds, ?string $system, ?int $anonymousRoleId) : array
    {
        $this->initializerb5509 && ($this->initializerb5509->__invoke($valueHolderedeb5, $this, 'findIdsWithGrantedPermissions', array('user' => $user, 'permission' => $permission, 'entityClass' => $entityClass, 'entityIds' => $entityIds, 'system' => $system, 'anonymousRoleId' => $anonymousRoleId), $this->initializerb5509) || 1) && $this->valueHolderedeb5 = $valueHolderedeb5;

        return $this->valueHolderedeb5->findIdsWithGrantedPermissions($user, $permission, $entityClass, $entityIds, $system, $anonymousRoleId);
    }
    ...
    public function __construct(\Doctrine\ORM\EntityManagerInterface $em, \Doctrine\ORM\Mapping\ClassMetadata $class)
    {
        ...
        if (! $this->valueHolderedeb5) {
            $reflection = $reflection ?? new \ReflectionClass('Sulu\\Bundle\\SecurityBundle\\Entity\\AccessControlRepository');
    ...
    public function & __get($name)
    {
        ...
        $realInstanceReflection = new \ReflectionClass('Sulu\\Bundle\\SecurityBundle\\Entity\\AccessControlRepository');
...
if (!\class_exists('AccessControlRepository_b331497', false)) {
    \class_alias(__NAMESPACE__.'\\AccessControlRepository_b331497', 'AccessControlRepository_b331497', false);
}
